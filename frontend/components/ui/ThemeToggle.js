...
export default function ThemeToggle({ inline = false }) {
  ...
  // Inline version for use in headers/navbars
  if (inline) {
    return (
      <button
        onClick={toggleTheme}
        className="p-2 text-slate-400 hover:text-white dark:text-slate-300 dark:hover:text-white transition-all duration-200 rounded-lg hover:bg-gradient-to-r hover:from-slate-700 hover:to-slate-600 hover:shadow-lg transform hover:scale-105"
