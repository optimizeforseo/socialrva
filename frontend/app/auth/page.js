  ...
  useEffect(() => {
    const handleLinkedInCallback = async () => {
      try {
        ...
        // Store LinkedIn access token and user ID for API calls
        if (data.accessToken) {
          localStorage.setItem("linkedinAccessToken", data.accessToken);
        }
        if (data.linkedinId) {
          localStorage.setItem("linkedinUserId", data.linkedinId);
        }

        // Show warning if profile data was limited
        if (data.warning) {
          console.warn("LinkedIn authentication warning:", data.warning);
        }
